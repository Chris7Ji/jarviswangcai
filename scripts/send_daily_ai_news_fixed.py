#!/usr/bin/env python3
"""
Fixed version of send_daily_ai_news_real.py
Fixes: proxy removal for Gemini, correct SMTP password
"""
import requests
import json
import smtplib
...
def fetch_real_news():
    ...
    try:
        ...
        if resp.status_code == 200:
            return resp.json().get('news_results', [])[:6]
    except Exception as e:
        print("SerpAPI Error:", e)
    return []

def call_gemini(prompt):
    """Gemini without proxy - direct connection"""
    ...
    # FIXED: No proxy - use direct connection
...
def translate_and_summarize(title, snippet):
    ...
    try:
        ...
        print(f"  JSON Parse Error: {e} | {res_text[:100]}")
...
def main():
    ...
    status = {"date": today_str, "stage": "start", "errors": []}
    
    # Step 1: Fetch news
    print(f"\n{'='*50}")
    print(f"[Step 1/3] Fetching news for {today_str}...")
    raw_news = fetch_real_news()
    if not raw_news:
        msg = "Failed to fetch news from SerpAPI"
        print(f"  ERROR: {msg}")
        status["stage"] = "fetch_failed"
        status["errors"].append(msg)
        with open(f"news_summaries/gaoxiao_status_{today_str}.json", "w") as f:
            json.dump(status, f, ensure_ascii=False, indent=2)
        return
        
    print(f"  ✓ Fetched {len(raw_news)} articles from SerpAPI")
    
    # Save raw JSON
    with open(f"news_summaries/gaoxiao_raw_{today_str}.json", "w") as f:
        json.dump(raw_news, f, ensure_ascii=False, indent=2)
    print(f"  ✓ Saved raw news to news_summaries/gaoxiao_raw_{today_str}.json")
    status["stage"] = "raw_saved"
    status["article_count"] = len(raw_news)
    ...
    # Step 2: Translate
    print(f"\n[Step 2/3] Translating {len(raw_news)} articles...")
    processed_news = []
    trans_success = 0
    trans_fail = 0
    for i, r in enumerate(raw_news):
        ...
        if trans['model'] != 'Failed':
            trans_success += 1
        else:
            trans_fail += 1
        time.sleep(1)
    
    print(f"  ✓ Translated: {trans_success} success, {trans_fail} failed")
    status["translated"] = trans_success
    status["trans_failed"] = trans_fail
    ...
    # Generate HTML
    print(f"\n[Step 2.5/3] Generating HTML...")
    html_content = generate_html_email(processed_news)
    html_path = f"news_summaries/gaoxiao_news_{today_str}.html"
    with open(html_path, "w") as f:
        f.write(html_content)
    html_size = len(html_content.encode())
    print(f"  ✓ HTML generated: {html_size} bytes ({trans_success} translated articles)")
    status["html_size"] = html_size
    ...
    print(f"\n[Step 3/3] Sending emails to {len(TO_EMAILS)} recipients...")
    success_count, fail_count = send_emails_batch(TO_EMAILS, subject, html_content)
    
    print(f"\n  ✓ Sent to {success_count}/{len(TO_EMAILS)} recipients")
    if fail_count > 0:
        print(f"  ✗ Failed: {fail_count}")
    status["email_sent"] = success_count
    status["email_failed"] = fail_count
    status["stage"] = "complete" if fail_count == 0 else "partial"
    status["recipients"] = len(TO_EMAILS)
    ...
    print(f"\n{'='*50}")
    print(f"✅ Task complete for {today_str}!")
    print(f"  Raw news: {len(raw_news)} articles")
    print(f"  Translated: {trans_success}/{len(raw_news)}")
    print(f"  HTML size: {html_size} bytes")
    print(f"  Email sent: {success_count}/{len(TO_EMAILS)}")
    print(f"{'='*50}")
