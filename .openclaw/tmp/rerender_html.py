#!/usr/bin/env python3
"""Standalone HTML generator - copies logic from auto_news_v2.generate_html without triggering side effects"""
import json
...
<body>
    <nav class="navbar">
        <div class="logo"><a href="index.html"><img src="images/logo.png" alt="logo"> 旺财Jarvis</a></div>
        <div class="nav-links">
    ...
    <script>
        function showDate(dateStr) {{
            document.querySelectorAll('.day-news-container').forEach(el => el.classList.remove('active'));
            document.querySelectorAll('.date-pill').forEach(el => el.classList.remove('active'));
            document.getElementById('news-' + dateStr).classList.add('active');
            document.querySelector(`.date-pill[data-date="${{dateStr}}"]`).classList.add('active');
        }}
    </script>
</body>
</html>"""
...
print(f"✅ HTML regenerated: {HTML_PATH} ({html_size} bytes)")
print(f"✅ {len(dates)} days rendered, latest: {dates[0]}")
